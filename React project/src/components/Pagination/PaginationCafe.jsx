...
const PaginationCafe = (props) => {
    ...
    const drawPage = (currentPosition, limit) => {
        ...
        let cafes = allElements.slice(currentPosition, currentPosition + limit);
        ...
        <div className={style.cafe_component}>
            <div>
                {/*<Suspense fallback={<div>Loading...</div>}>*/}
                    {
                        items.map((el, index) => <Cafe elem={el} key={el.id} /> )
                    }
                {/*</Suspense>*/}
            </div>
            <ReactPaginate
